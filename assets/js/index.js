...
// Xử lý nút Confirm (Cho phép luôn)
$("body").on("click", ".confirm-btn", function(){
    if(window.__playing) return;
    window.__playing = true;
    $("#toast-prompt").slideUp("fast", function(){
        let exp = new Date(Date.now() + 86400000).toUTCString(); // 1 ngày
        document.cookie = `toast=close; expires=${exp}; path=/`;
    });
    playRandomMusic();
});

// Xử lý nút Close (Không hiển thị lại)
$("body").on("click", ".close-btn", function(){
    $("#toast-prompt").slideUp("fast", function(){
        let exp = new Date(Date.now() + 600000).toUTCString(); // 10 phút
        document.cookie = `toast=close; expires=${exp}; path=/`;
    });
